...
void UHumanAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	groundSpeed = humanCharacter->GetVelocity().Size2D();
	FVector velocity = humanCharacter->GetVelocity();
	direction = FMath::FindDeltaAngleDegrees(humanCharacter->GetActorRotation().Yaw, velocity.ToOrientationRotator().Yaw);
	
	//공중 여부 설정
	isAir = humanCharacter->GetMovementComponent()->IsFalling();
	
	auto* Inventory = humanCharacter->inventoryComponent;
